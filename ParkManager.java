...
public class ParkManager extends User implements Serializable{	
	...
	public boolean isJobTooSoonToCancel(LocalDate theStart) {
		LocalDate closestToCancel = theStart.minusDays(MAX_CANCEL);
		
		if(LocalDate.now().isEqual(closestToCancel) || LocalDate.now().isAfter(closestToCancel)){
