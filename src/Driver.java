...
public class Driver {
    		...
    		for(int i = 0; i<theUsers.size(); i++) {
    			if (theUsers.get(i).getName().equals(name)) {
    				...
    				if(theUsers.get(i).getType().equals("Manager")) {
    					...
    					Volunteer.identifyVolunteer(theUsers.get(i));
    					
    					Volunteer myVolunteer = (Volunteer) (theUsers.get(i));
    					showVolunteerMenu(myVolunteer, theUsers, theJobs);
    				} else if(theUsers.get(i).getType().equals("Urban Parks Employee")) {
    					UrbanParksEmployee.identifyUrbanParksEmployee(theUsers.get(i));
    					
    					UrbanParksEmployee myEmployee = (UrbanParksEmployee) (theUsers.get(i));
    					showUrbanParksEmployeeMenu(myEmployee, theUsers, theJobs);
    	    		}
    	    	}
		    }
		}
	...
	public static void showUrbanParksEmployeeMenu(UrbanParksEmployee theEmployee, 
			ArrayList<User> theUsers, ArrayList<Job> theJobs) {
		System.out.println("Please choose from one of the following (1-3): ");
		...
		if (choice.equals("1")) {
			...
			System.out.println();
			showUrbanParksEmployeeMenu(theEmployee, theUsers, theJobs);
		} else if (choice.equals("3")) {
			signIn(theUsers, theJobs);
		} else {
			System.out.println("You did not input a valid answer so the menu "
