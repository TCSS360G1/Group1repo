...
public class ParkManagerMenuBar extends JMenuBar {
	...
	private void current() {
		...
		ViewCurrent.addActionListener(new ActionListener() {
			...
			public void actionPerformed(final ActionEvent theEvent) {
				...
				add(new ParkManagerDisplayCurrentJobs(myManager),
						BorderLayout.CENTER);
			}
		});
	...
	private void newJob() {
		...
		NewJob.addActionListener(new ActionListener() {
			...
			public void actionPerformed(final ActionEvent theEvent) {
				...
				add(new ParkManagerNewJobPanel(myManager),
						BorderLayout.CENTER);
	...
	private void updates() {
		...
		Updates.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(final ActionEvent theEvent) {
				//this is passing in entire job class. we want to only
				//pass in jobs that are available to be cancelled.
				//updatesPanel(myManager);
				new ParkManagerCancelJobsPanel(myManager,
						Job.filterForCancellation(myManager.getJobs()));
				add(new ParkManagerCancelJobsPanel(myManager,
						Job.filterForCancellation(myManager.getJobs())),
						BorderLayout.CENTER);
			}
		});
