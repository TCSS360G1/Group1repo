...
public class SignInPanel extends JPanel implements PropertyChangeListener {
	...
	/**
     * precondition: myUsernameText != null and mySignInButton != null.
     * 
    ...
    /**
     * precondition: This SignInPanel != null.
     * 
     * postcondition: This method allows a User to sign in, and their
     * respective User type Panel will be shown to them.
     */
...
//		}
    		...
    		for(int i = 0; i<theUsers.getSize(); i++) {
    			...
    			if (theUsers.getIndex(i).getName().toLowerCase()
    				...
    				if(theUsers.getIndex(i).getType().equals("Manager")) {
    					...
    					ParkManager manager = (ParkManager)
    							theUsers.getIndex(i);
    					System.out.println(manager.getJobs().get(0));
    					managerPanel = new ParkManagerPanel(manager,
    							myJobs.filterPast());
    ///////////////////	ADDED LISTENERS HERE
    					...
    					VolunteerPanel volunteerPanel =
    							new VolunteerPanel((Volunteer)
    									myUsers.getIndex(i), myJobs);
    					...
    					UrbanParksEmployee employee = (UrbanParksEmployee)
    							(myUsers.getIndex(i));
    					UrbanParksPanel employeePanel =
    							new UrbanParksPanel(theJobs);
	...
	public void propertyChange(PropertyChangeEvent evt) {
		if(evt.getPropertyName().equals("Manager add")) {
			...
			System.out.println(myJobs.getSize()
					+ " " + manager.getJobs().size());
		} else if(evt.getPropertyName().equals("Manager remove")) {
			System.out.println("ORIGINAL BEFORE REMOVING SIZE "
		            + myJobs.getSize());
			ParkManager manager = (ParkManager) evt.getOldValue();
			//delete the job
